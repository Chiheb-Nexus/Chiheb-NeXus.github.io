...
if __name__ == '__main__':
	import os
	s = int(binascii.hexlify(os.urandom(32)), 16)
	a = Base58()
	encoded = a.encode(s)
	decoded = a.decode(encoded)
	# test: if not equal throw AssertionError
	assert s == int.from_bytes(decoded, byteorder='big')
	print('input: {0}\nencoded: {1}\ndecoded: {2}'.format(
		s, encoded, int.from_bytes(decoded, byteorder='big'))
	)
